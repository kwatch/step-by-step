...
##
## subject: URLパスパターンを正規表現にコンパイルする
##
...
## URLパスパターンを正規表現に変換する。
## 例: '/api/users.json' → '^/api/users\\.json$'
def _convert_urlpath(urlpath):  # ex: '/api/users.json'
    def _re_escape(urlpath):
        return re.escape(urlpath).replace(r'\/', '/')
    #
    buf = ['^']; add = buf.append
    add(_re_escape(urlpath))    # ex: '/api/users\\.json'
    add('$')
    return "".join(buf)         # ex: '^/api/users\\.json$'
